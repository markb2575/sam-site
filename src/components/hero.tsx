...
import { Mail, PhoneIcon, CalendarDays } from "lucide-react";
  ...
  return (
    <section className="relative min-h-screen w-full flex items-center justify-center overflow-hidden rounded-none shadow-none">
      ...
      <div className="relative z-20 flex flex-col items-center justify-center text-center text-white px-4 gap-10">
        <h1 className="text-5xl md:text-6xl font-extrabold drop-shadow mb-2 mt-8 md:mt-0">{companyInfo.name}</h1>
        <h2 className="text-2xl md:text-3xl font-semibold text-gray-200 mb-6 drop-shadow">{companyInfo.type}</h2>
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <a className="flex items-center gap-2 text-lg hover:underline" href={`mailto:${contactInfo.email}`}>
            <Mail/> {contactInfo.email}
          </a>
          <a className="flex items-center gap-2 text-lg hover:underline" href={`tel:${contactInfo.phone.replace(/[^\d+]/g, '')}`}>
            <PhoneIcon /> {contactInfo.phone}
          </a>
          <a className="flex items-center gap-2 text-lg hover:underline" href={contactInfo.calendar} target="_blank" rel="noopener noreferrer">
